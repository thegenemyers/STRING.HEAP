...
#ifdef STATS
#ifdef RATS
  printf(" & %.1f",slcp/(1.*sin));
  printf(" & %.1f",sin/(1.*sout));
  printf(" & %d%%",(int) (sin/(.01*sout*T)));
#else
  printf(", %.1f",slcp/(1.*sin));
  printf(", %.3f",sin/(1.*sout));
#endif
  fflush(stdout);
#endif
}
...
{ int r, t, R;
  ...
  if (argc < 4)
    { fprintf(stderr,"Usage: Sheap <R:int> <out:file> <in:file> ...\n");
      exit (1);
    }
